...
# m52 = ConstantKernel(constant_value=0.2,constant_value_bounds=(1e-4, 1e4)) * Matern(length_scale=1.0, nu=2.5)
...
class UtilityFunction(object):
    ...
    """
    def __init__(self, kind, kappa, x_i, kappa_decay=1, kappa_decay_delay=0):
        self.kappa = kappa
        self.x_i = x_i
...
class Searcher(AbstractSearcher):
    ...
    def acq_max(self, f_acq, gp, y_max, bounds, num_warmup, num_starting_points):
        ...
        #                 size=(num_warmup, bounds.shape[0]))
        # Warm up with random points
    ...
    def suggest(self, suggestions_history, n_suggestions=1):
        ...
        if (suggestions_history is None) or (len(suggestions_history) <= 0):
            ...
            for index in range(n_suggestions):
              # kappa=(index + 1) * 2.576 , x_i=index * 20
                utility_function = UtilityFunction(kind='ei', kappa=(index + 1) * 2.576 * 1.5, x_i=index * 6)
